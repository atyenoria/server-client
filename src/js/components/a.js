...
export default class WelcomePage extends Component {
    ...
    componentDidMount() {
        ...
        socket.on('connect', function() {
            socket.emit('authenticate', {
                token: "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJmb28iOiJiYXIiLCJpYXQiOjE0NDgzNzU0MjJ9.D36z6zz-gFncPbVOdMznqXGUeDhgBsfn1sLmxJGnICk"
            });
        });
    ...
    handleEmitSubmit(event) {
        ...
        socket.emit('id msg', [{
            id: this.state.isocket_id
        }, {
            id: this.state.emit_body
        }]);
        this.setState({
            isocket_id: '',
            emit_body: ''
    ...
    handleChangeBody(event) {
        this.setState({
            emit_body: event.target.value
        });
    }
    handleChangeSocketId(event) {
        this.setState({
            isocket_id: event.target.value
        });
    }
    ...
    handleChangeEventName(event) {
        this.setState({
            emit_event_name: event.target.value
        });
    }
    handleChangeEventBody(event) {
        this.setState({
            emit_event_body: event.target.value
        });
    }
    ...
    render() {
        ...
        return ( < div >
            ...
            } > Socket.io Test < /p> < /header>


            < Button bsStyle = "success"
            onClick = {::this.RestartSocket
            }
            type = "submit" >
            < p style = {
                {
            ...
            } > Restart Socket < /p> < /Button> < Button bsStyle = "success" >
            < p style = {
                {
                    color: 'white',
                    margin: '0',
                ...
                this.state.mysocket_id
            } < /p> < /Button>




            < Input label = "isocket_id"
            ref = "usernameInput"
            type = "text"
            style = {
                {
            ...
            value = {
                this.state.isocket_id
            }
            onChange = {::this.handleChangeSocketId
            }
            /> < Input label = "emit_body"
            ref = "usernameInput"
            style = {
                {
                    width: '50%'
            ...
            < Input label = "emit_event_name"
            ref = "usernameInput"
            type = "text"
            name = "username"
            placeholder = "Enter event name"
            value = {
                this.state.emit_event_name
            }
            onChange = {::this.handleChangeEventName
            }
            /> < Input label = "emit_event_body"
            ref = "usernameInput"
            style = {
                {
                    width: '50%'
